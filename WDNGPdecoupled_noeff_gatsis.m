...
filename = ['WaterDecoupled_noeff_PRV_',num2str(WDN_num),'.mat'];
else
end
cd('..')
%% WDN side Quantities
...
IntValveOpen_iter = ones(Valves,Time);
IntCp_prv = zeros(Valves, Time);
for i=1:length(Valves)
IntCp_prv(i,:) = IntFlowValves(i,:).*((Omega_prv(i,i)./(IntValveOpen_iter(i,:).^2)).*abs(IntFlowValves(i,:))-(ones(1,Time)./(IntValveOpen_iter(i,:))));
end
...
IntEps = [IntHeads;IntFlows;IntOnOff;IntValveStatusOnOff];
M_pump = 3e03;
M_valve = 4e03; 
Cp = [] ;
C1M = [];
...
%(Cp_prv.*ValveOpen+Zeta'*Flows)>=0
%0<=ValveSubStatusy<=ValveStatusOnOff;
%0<=ValveSubStatusz<=ValveStatusOnOff;
ValveSubStatusy+ValveSubStatusz==ValveStatusOnOff;
M_valve.*(ValveSubStatusy-1)+ValveSetting<=Heads(5,:)<=M_valve.*(1-ValveSubStatusz)+ValveSetting;
M_valve.*(ValveSubStatusy-1)+ValveSetting<=Heads(7,:)<=M_valve.*(1-ValveSubStatusy)+ValveSetting;
0.*(ValveStatusOnOff)<=Zeta'*Flows<=(Zeta'*MaxFlowPump).*ValveStatusOnOff;
%0.001*ValveStatusOnOff<=ValveOpen<=ones(Valves,Time).*ValveStatusOnOff; % open constraint
%0<=ValveOpen<=M_valve.*(1-ValveSubStatusz); % open constraint
0.000001.*ValveSubStatusy+0.*ValveSubStatusz<=ValveOpen<=0.*ValveSubStatusz+M_valve.*ValveSubStatusy
cvx_end
...
% for i=1:length(Valves)
% Cp_prv(i,:) = FlowValves(i,:).*(((Omega_prv(i,i)./(ValveOpen(i,:).^2)).*abs(FlowValves(i,:)))-(ones(1,Time)./(ValveOpen(i,:))));
% end
...
Eps = [Heads;Flows;OnOff;ValveStatusOnOff];
Obj = cvx_optval;
